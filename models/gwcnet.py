...
class feature_extraction(nn.Module):
    ...
    def forward(self, x):
        ...
        guidance = self.guidance(gwc_feature)
        
        guidance_l, _ = torch.split(guidance, [batch_dim, batch_dim], dim=0)
        gwc_feature_left, gwc_feature_right = torch.split(gwc_feature, [batch_dim, batch_dim], dim=0)
        concat_feature_left, concat_feature_right = torch.split(concat_feature, [batch_dim, batch_dim], dim=0)

        return guidance_l, {"gwc_feature": gwc_feature_left, "concat_feature": concat_feature_left}, \
               {"gwc_feature": gwc_feature_right,
                "concat_feature": concat_feature_right}
...
class PropgationNet_1x(nn.Module):
    def __init__(self, input_channel):
        ...
        self.mask = nn.Sequential(convbn(input_channel, input_channel * 2, 3, 1, 1, 1),
                                  nn.ReLU(inplace=True),
                                  nn.Conv2d(input_channel * 2, 9 * 16, kernel_size=(3, 3), stride=(1, 1), padding=1,
                                            dilation=(1, 1), bias=False))

    def forward(self, guidance, disp):
        b, c, h, w = disp.shape
        disp = F.unfold(4 * disp, [3, 3], padding=1).view(b, 1, 9, 1, 1, h, w)
        mask = self.mask(guidance).view(b, 1, 9, 4, 4, h, w)
...
class GwcNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        guidance, features_left, features_right = self.feature_extraction(
        ...
        prob3 = F.softmax(cost3, dim=1).squeeze(1)

        pred3_4x = disparity_regression(prob3, self.maxdisp//4, keepdim=True)
        pred3 = self.up(guidance, pred3_4x)
